...
def generate_payroll_pdf(
    ...
    if logo_path and logo_path.exists():
        logo = Image(str(logo_path), width=2.6*cm, height=2.6*cm)
    else:
        logo = ""
    ...
    header = Table([
        [
            logo,
            Paragraph(f"""
                <font color="white" size="15"><b>LIVING LOUNGE S.A.</b></font><br/>
            ...
            Paragraph(f"""
                <para align=center>
                <font color="white" size="12"><b>COMPROBANTE DE PAGO</b></font><br/><br/>
    ...
    empleado_table = Table([
        [
            Paragraph(f"<b>Trabajador</b><br/>{empleado['nombre']}", styles["BodyText"]),
            Paragraph(f"<b>Puesto</b><br/>{empleado['puesto']}", styles["BodyText"])
        ]
    ], colWidths=[250, 250])
    ...
    # ---------- KPI ----------
    kpi = Table([
        ["SALARIO MENSUAL", "COSTO HORA"],
        [currency(salario_mensual), currency(salario_hora)]
    ], colWidths=[255, 255])
    ...
    detalle = [
        ...
        ["1008","CCSS SEM","",currency(sem)],
        ["1009","CCSS IVM","",currency(ivm)],
        ["1010","Banco Popular","",currency(banco)],
        ["1011","Renta","",currency(renta)],
        ["1012","Embargos","",currency(embargos)]
    ]

    detalle_table = Table(detalle, colWidths=[60,240,105,105])
    ...
    footer = Paragraph("""
        ...
        Documento generado automáticamente por Sistema RRHH — Living Lounge S.A.
        </font>
        </para>
    """, styles["Normal"])
